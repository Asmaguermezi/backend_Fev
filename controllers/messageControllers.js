...
// ✅ Créer un message (API REST, non socket)
...
// ✅ Récupérer tous les messages liés à une session (pour VideoCall)
exports.getMessagesBySessionId = async (req, res) => {
  try {
    const sessionId = req.params.sessionId;
    const messages = await Message.find({ sessionId }).sort({ timestamp: 1 });
    res.status(200).json(messages);
  } catch (error) {
    res.status(500).json({ error: "Erreur lors de la récupération des messages de session" });
  }
};
